...
for y in range(H):
    for x in range(W):
        if not fg[y, x]:
            continue
        # 1. Top Bridge (Upper Frame Arch & Guide Columns in assembled state)
...
# Vector displacements away from exact assembled (0, 0):
components = {
    'top_bridge': {
        'offset': (0, -140),
        'start_p': 0.02,
...
# Extract isolated components directly from the real assembled machine
...
# Helper function to draw subtle CAD alignment guide lines
...
for i in range(TOTAL_FRAMES):
    ...
    # Calculate displacement for each component away from assembled (0,0)
    displacements = {}
    progresses = {}
    for name in render_order:
        cfg = components[name]
    ...
    # Draw subtle engineering CAD alignment guide lines when parts are separated
    top_t = progresses['top_bridge']
    if top_t > 0.08:
        alpha = int(min(1.0, (top_t - 0.08) / 0.3) * 150)
        c = (197, 34, 39, alpha)
        top_y_ass = 350
        top_y_exp = 350 + displacements['top_bridge'][1]
        draw_dashed_line(draw, (565, top_y_ass), (565, top_y_exp), c, dash_len=6, gap_len=5, width=2)
        draw_dashed_line(draw, (810, top_y_ass), (810, top_y_exp), c, dash_len=6, gap_len=5, width=2)
    ...
    if left_t > 0.08:
        ...
        c = (197, 34, 39, alpha)
        left_x_ass = 500
        left_x_exp = 500 + displacements['left_assembly'][0]
        draw_dashed_line(draw, (left_x_ass, 380), (left_x_exp, 380), c, dash_len=6, gap_len=5, width=2)
        draw_dashed_line(draw, (left_x_ass, 490), (left_x_exp, 490), c, dash_len=6, gap_len=5, width=2)
    ...
    if right_t > 0.08:
        ...
        c = (197, 34, 39, alpha)
        right_x_ass = 870
        right_x_exp = 870 + displacements['right_assembly'][0]
    ...
    if base_t > 0.08:
        ...
        c = (197, 34, 39, alpha)
        base_y_ass = 560
        base_y_exp = 560 + displacements['base_table'][1]
...
print(f"SUCCESS: Generated {TOTAL_FRAMES} perfectly continuous master CAD exploded frames!")
