...
# Continuous Staggered Timeline:
# 0.00 (Stage 01) = Exact 100% Assembled Machine (fixture_assembled.png)
# 0.25 (Stage 02) = Slightly Exploded
# 0.50 (Stage 03) = Moderately Exploded
# 0.75 (Stage 04) = Mostly Exploded
# 1.00 (Stage 05) = Clean 3D Exploded View
components = {
    'top_bridge': {
        'offset': (0, 140),
        'start_p': 0.04,
...
for i in range(TOTAL_FRAMES):
    ...
    if i == 0:
        # Frame 00 is 100% bit-for-bit the exact complete assembled machine!
        ...
        # Seamlessly crossfade with exact assembled master at the very start (p < 0.06)
        if p < 0.06:
            blend_factor = (0.06 - p) / 0.06
            canvas = Image.blend(canvas, img_ass, blend_factor)
            
        frame_img = canvas
        
    out_path = f'public/images/cad_dense_sequence/frame_{i:02d}.png'
    canvas_to_save = frame_img if i == 0 else frame_img
    canvas_to_save.save(out_path, 'PNG', compress_level=1)

print(f"SUCCESS: Generated {TOTAL_FRAMES} frames with Frame 00 being 100% exact assembled machine!")
