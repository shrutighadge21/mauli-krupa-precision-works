...
# Continuous Staggered Timeline mapped to 5 Clear Milestones:
# 0.00 (Stage 01) = 0% Exploded (100% Assembled Machine)
# 0.25 (Stage 02) = 25% Exploded (Slightly Exploded: Top & Left begin moving)
# 0.50 (Stage 03) = 50% Exploded (Moderately Exploded: Right begins, Top & Left farther)
# 0.75 (Stage 04) = 75% Exploded (Mostly Exploded: Base lowers, all 4 parts clearly separated)
# 1.00 (Stage 05) = 100% Exploded (Clean Compact 3D Exploded Engineering Assembly)
components = {
    'top_bridge': {
        'offset': (0, 140),
        'start_p': 0.04,
...
# Extract isolated organic 3D CAD parts
parts = {}
...
for i in range(TOTAL_FRAMES):
    ...
    for name in render_order:
        ...
        # ox, oy: displacement relative to exploded (0, 0)
    ...
    if top_t > 0.05:
        alpha = int(min(1.0, (top_t - 0.05) / 0.3) * 150)
        c = (197, 34, 39, alpha)
    ...
    if left_t > 0.05:
        alpha = int(min(1.0, (left_t - 0.05) / 0.3) * 150)
        c = (197, 34, 39, alpha)
    ...
    if right_t > 0.05:
        alpha = int(min(1.0, (right_t - 0.05) / 0.3) * 150)
        c = (197, 34, 39, alpha)
    ...
    if base_t > 0.05:
        alpha = int(min(1.0, (base_t - 0.05) / 0.3) * 150)
        c = (197, 34, 39, alpha)
        base_y_ass = 540
